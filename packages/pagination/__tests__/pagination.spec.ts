...
import { sleep } from '@missfresh/test-utils'
...
describe('Pagination.vue', () => {
  ...
  test('change layout value', async () => {
    ...
    expect(
      wrapper
        .find('.el-pagination__rightwrapper .el-pagination__jump')
        .exists(),
    ).toBe(true)
    ...
    expect(
      wrapper
        .find('.el-pagination__rightwrapper .el-pagination__sizes')
        .exists(),
    ).toBe(true)
    ...
    expect(
      wrapper
        .find('.el-pagination__rightwrapper .el-pagination__total')
        .exists(),
    ).toBe(true)
  })
...
describe('click pager', () => {
  ...
  test('should emit change size evt and update pageSize', async () => {
    ...
    const items = document
      .querySelector('.select-dropdown-klass')
      .querySelectorAll('.el-select-dropdown__item:not(.selected)')
    ;(items[0] as HTMLOptionElement)?.click()
    expect(onSizeChange).toHaveBeenCalled()
    expect(wrapper.vm.pageSize).toBe(100)
    expect(wrapper.findComponent(Pagination).emitted()).toHaveProperty(
      'size-change',
    )
  })

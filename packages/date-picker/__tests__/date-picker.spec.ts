import { CommonPicker } from '@missfresh/time-picker'
import { mount } from '@vue/test-utils'
import dayjs from 'dayjs'
import 'dayjs/locale/zh-cn'
import { nextTick } from 'vue'
import DatePicker from '../src/date-picker'
import Input from '@missfresh/input'
...
describe('DatePicker', () => {
  ...
  describe('value-format', () => {
    it('with literal string', async () => {
      ...
      const valueFormat = '[missfresh] DD/MM YYYY'
      ...
      `,
        ...
        },
        {
          methods: {
            changeValue() {
              this.value = '[missfresh] 31/05 2021'
            },
          },
        },
      )
      ...
      expect(vm.value).toBe(
        dayjs(
          `[missfresh] 01/${('0' + (day.month() + 1)).slice(-2)} ${day.year()}`,
          valueFormat,
        ).format(valueFormat),
      )
...
describe('MonthPicker', () => {
  ...
  it('value-format', async () => {
    const valueFormat = '[missfresh] YYYY.MM'
    const wrapper = _mount(
      `
      <el-date-picker
        type="month"
...
describe('YearPicker', () => {
  ...
  it('value-format', async () => {
    const valueFormat = '[missfresh] YYYY'
    const wrapper = _mount(
      `
      <el-date-picker
        type="year"

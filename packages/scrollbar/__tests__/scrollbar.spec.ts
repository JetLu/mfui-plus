...
import { defineGetter, makeScroll } from '@missfresh/test-utils'
...
describe('ScrollBar', () => {
  test('vertical', async () => {
    ...
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain(
      'height: 40%; transform: translateY(50%); webkit-transform: translateY(50%)',
    )
    await makeScroll(scrollDom, 'scrollTop', 300)
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain(
      'height: 40%; transform: translateY(150%); webkit-transform: translateY(150%)',
    )
    clientHeightRestore()
    scrollHeightRestore()
  })
  ...
  test('horizontal', async () => {
    ...
    const clientWidthRestore = defineGetter(
      scrollDom,
      'clientWidth',
      outerWidth,
    )
    ...
    expect(wrapper.find('.is-horizontal div').attributes('style')).toContain(
      'width: 40%; transform: translateX(50%); webkit-transform: translateX(50%)',
    )
    await makeScroll(scrollDom, 'scrollLeft', 300)
    expect(wrapper.find('.is-horizontal div').attributes('style')).toContain(
      'width: 40%; transform: translateX(150%); webkit-transform: translateX(150%)',
    )
    clientWidthRestore()
    scrollWidthRestore()
  })
  ...
  test('set scrollTop & scrollLeft', async () => {
    ...
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain(
      'height: 40%; transform: translateY(0%); webkit-transform: translateY(0%);',
    )
    expect(wrapper.find('.is-horizontal div').attributes('style')).toContain(
      'width: 40%; transform: translateX(0%); webkit-transform: translateX(0%);',
    )

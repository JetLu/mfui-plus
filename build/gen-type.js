  ...
  if (!noElPrefixFile.test(comp)) {
    if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
      ...
      const imp = fs
        ...
        .toString()
      if (outsideImport.test(imp) || imp.includes('@missfresh/')) {
        const newImp = imp
          .replace(outsideImport, (i, c) => {
            return i.replace(`../${c}`, `../mf-${c}`)
          })
          .replace(/@missfresh\//g, '../mf-')
          .replace('el-utils', 'utils')
          .replace('el-locale', 'locale')
